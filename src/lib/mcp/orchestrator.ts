...
export class AgentOrchestrator {
    ...
    async runFullPipeline(): Promise<{
        ...
        if (aiEngine.isConnected) {
            try {
                testPlan = await aiEngine.generateTestPlan(
                    codeContext,
                    effectiveFiles,
        ...
        try {
            ...
            const prTitle = hasHealerFix
                ? selectedPr
                    ? `[SentinelQA] Fix for PR #${selectedPr.number} (${healerOutput!.rca_type}) — ${finalResults.passed}/${finalResults.total} passed${postFixResults ? " after fix" : ""}`
                    : `[SentinelQA] Auto-fix + E2E tests (${healerOutput!.rca_type}) — ${finalResults.passed}/${finalResults.total} passed${postFixResults ? " after fix" : ""}`
                : selectedPr
            ...
            if (ghToken) {
                try {
                    ...
                    if (hasHealerFix && postFixResults) {
                        commentBody = `### 🛡️ SentinelQA — Fix Ready\n\n`
                          + `Fix PR: ${prResult.url}\n\n`
                        ...
                        commentBody = `### 🛡️ SentinelQA — Tests Generated\n\n`
