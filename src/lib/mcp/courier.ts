...
export class CourierAgent {
  ...
  async createFixPR(input: CourierPRInput): Promise<CourierResult> {
    try {
      ...
      const prBody = `## 🛡️ SentinelQA Auto-Fix
...
---
*This PR was created automatically by the SentinelQA Courier agent.*`;
  ...
  async createIssueReport(input: CourierIssueInput): Promise<CourierResult> {
    try {
      ...
      const issueBody = `## 🚨 SentinelQA — Test Failure Report
...
---
*This issue was created automatically by the SentinelQA Courier agent.*`;
  ...
  async dispatch(
    ...
    if (confidence >= 0.8) {
      return this.createFixPR({
        ...
        repo,
        base_branch: branch,
        head_branch: `sentinelqa/fix-${sessionId}`,
        title: `[SentinelQA] ${title}`,
        body,
