// ============================================================================
// SentinelQA — WebSocket Server (Socket.io)
//
...
type GlobalWithIO = typeof globalThis & {
  __sentinelqa_io?: SocketIOServer;
  __sentinelqa_http?: HTTPServer;
};
...
export function getIO(httpServer?: HTTPServer): SocketIOServer | null {
  if (g.__sentinelqa_io) return g.__sentinelqa_io;

  const server = httpServer ?? g.__sentinelqa_http;
  if (!server) {
    // No HTTP server available yet — queue events will be no-ops
    return null;
  }

  g.__sentinelqa_http = server;
  g.__sentinelqa_io = new SocketIOServer(server, {
    path: "/api/ws",
    cors: { origin: "*", methods: ["GET", "POST"] },
    transports: ["websocket", "polling"],
  });

  g.__sentinelqa_io.on("connection", (socket) => {
